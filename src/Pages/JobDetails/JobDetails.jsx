...
const JobDetails = () => {
    ...
    const {_id,job_title,category,deadline,description,min_price,max_price,buyer_email,buyer_name,buyer_photo} = job
    ...
    const handleBid = e => {
        ...
        const deadlineOffer = startDate
        if(price < parseInt(min_price)){
            return toast.error('Please Add Minimum Price At least')
        }
        else if(price>parseInt(max_price)){
            return toast.error('You have to Charge below the Max Price')
        }
        const bidData = {
            jobId,price,buyerEmail,comment,category,status,email,deadlineOffer,job_title
        }
        axios.post(`${import.meta.env.VITE_URL}/bids`,bidData)
        .then( res => {
